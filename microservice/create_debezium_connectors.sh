...
sh infra/debezium/create_domain_events_topics.sh

ASSGINMENTS_CONNECTOR_JSON="$(cat infra/debezium/connector-config-assignments.json)"
...
AVAILABILITIES_CONNECTOR_JSON="$(cat infra/debezium/connector-config-availabilities.json)"
...
FIXTURES_CONNECTOR_JSON="$(cat infra/debezium/connector-config-fixtures.json)"
...
REFEREES_CONNECTOR_JSON="$(cat infra/debezium/connector-config-referees.json)"
...
TEAMS_CONNECTOR_JSON="$(cat infra/debezium/connector-config-teams.json)"
echo "TEAMS_CONNECTOR_JSON: $TEAMS_CONNECTOR_JSON"
curl -X POST \
    'http://localhost:8083/connectors' \
    --header 'Accept: */*' \
    --header 'User-Agent: Thunder Client (https://www.thunderclient.com)' \
    --header 'Content-Type: application/json' \
    --data-raw "$TEAMS_CONNECTOR_JSON"

VENUES_CONNECTOR_JSON="$(cat infra/debezium/connector-config-venues.json)"
